...
data_config = dict(image_size=[900,1600], input_size=[512,1408], render_size=[256,704])
resize_config = dict(input_scale=0.88, render_scale=0.44)
num_rays = [32, 88]
amp = False
flow = True
tracking = True
...
num_gpus = 8
label_path = {
    'semantic':'data/nuscenes/nuscenes_semantic',
    'flow':'data/nuscenes/nuscenes_flow',
}
iters_per_epoch = int(3516*8/num_gpus)
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'img_neck':dict(lr_mult=0.25),
            'lifter':dict(lr_mult=0.25), 'encoder':dict(lr_mult=0.25),
            'decoder':dict(lr_mult=0.25),
            'temporal_fusion':dict(lr_mult=0.4),
            'temporal_encoder_backbone':dict(lr_mult=0.4), 'temporal_encoder_neck':dict(lr_mult=0.4),
            'head.model.field.density_net':dict(lr_mult=0.4)
            }
    ),
)
...
train_dataset_config = dict(
    _delete_=True,
    type='nuScenes_Sweeps_Dist_Lidar',
    split='train',
    data_path = data_path,
    ...
    loss_cfgs=[
        dict(
            ...
            input_dict={
                ...
                'points_warps': 'points_warps',
                'static_flows': 'static_flows',
...
load_from = 'ckpts/nuscenes_occ_odo_static_state.pth'
