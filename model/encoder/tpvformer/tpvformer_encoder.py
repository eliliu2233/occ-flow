...
class TPVFormerEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        size_h = self.mapping.size_h if is_corner else self.mapping.size_h - 1
        size_w = self.mapping.size_w if is_corner else self.mapping.size_w - 1
        size_d = self.mapping.size_d if is_corner else self.mapping.size_d - 1
        ...
        if is_corner:
            ...
            hw_grid = torch.stack([torch.arange(size_h, dtype=torch.float).unsqueeze(-1).expand(-1, size_w)+0.5,
                torch.arange(size_w, dtype=torch.float).unsqueeze(0).expand(size_h, -1)+0.5,
                torch.zeros(size_h, size_w)], dim=-1)
        ...
        if is_corner:
            ...
            zh_grid = torch.stack(
                [torch.arange(size_h, dtype=torch.float).unsqueeze(0).expand(size_d, -1)+0.5,
                torch.zeros(size_d, size_h),
                torch.arange(size_d, dtype=torch.float).unsqueeze(-1).expand(-1, size_h)+0.5],
                dim=-1)
        ...
        if is_corner:
            ...
            wz_grid = torch.stack(
                [torch.zeros(size_w, size_d),
                torch.arange(size_w, dtype=torch.float).unsqueeze(-1).expand(-1, size_d)+0.5,
                torch.arange(size_d, dtype=torch.float).unsqueeze(0).expand(size_w, -1)+0.5],
                dim=-1)
        ...
        uniform_d = torch.linspace(0, size_d - 1, num_points_cross[2]) if is_corner else torch.linspace(0.5, size_d - 0.5, num_points_cross[2])
        ...
        uniform_w = torch.linspace(0, size_w - 1, num_points_cross[1]) if is_corner else torch.linspace(0.5, size_w - 0.5, num_points_cross[1])
        ...
        uniform_h = torch.linspace(0, size_h - 1, num_points_cross[0]) if is_corner else torch.linspace(0.5, size_h - 0.5, num_points_cross[0])
        ...
        cross_view_ref_points = get_cross_view_ref_points(size_h, size_w, size_d, num_points_self) #, offset=(not is_corner)*0.5
