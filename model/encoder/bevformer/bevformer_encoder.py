...
class BEVFormerEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        size_h = self.mapping.size_h - 1
        size_w = self.mapping.size_w - 1
        size_d = self.mapping.size_d - 1
        bev_grid = torch.stack(
            [torch.arange(size_h, dtype=torch.float).unsqueeze(-1).expand(-1, size_w)+0.5,
            torch.arange(size_w, dtype=torch.float).unsqueeze(0).expand(size_h, -1)+0.5], dim=-1)
        ...
        uniform_d = torch.linspace(0, size_d - 1, num_points_cross) + 0.5
        ...
        bev_grid_normed[..., 0] = bev_grid_normed[..., 0] / (size_h)
        bev_grid_normed[..., 1] = bev_grid_normed[..., 1] / (size_w)
