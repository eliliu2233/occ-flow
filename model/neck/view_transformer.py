...
from utils.bev_pool_v2.bev_pool import bev_pool_v2
...
class LSSViewTransformerFunction3D(BaseModule):
    ...
    def __init__(
        ...
        self.grid_config = grid_config
        # dx, bx, nx = gen_dx_bx(self.grid_config['x'],self.grid_config['y'],self.grid_config['z'],)
        # self.dx = nn.Parameter(dx, requires_grad=False)
        # self.bx = nn.Parameter(bx, requires_grad=False)
        # self.nx = nn.Parameter(nx, requires_grad=False)
    ...
    def create_grid_infos(self, x, y, z, **kwargs):
        ...
        self.grid_lower_bound = torch.Tensor([cfg[0] for cfg in [x, y, z]])
        self.grid_interval = torch.Tensor([cfg[2] for cfg in [x, y, z]])
        self.grid_size = torch.LongTensor([(cfg[1] - cfg[0]) / cfg[2]
                                       for cfg in [x, y, z]])
    ...
    def get_lidar_coor(self, rots, trans, cam2imgs, img_aug, bda):
        """Calculate the locations of the frustum points in the lidar
        coordinate system.
        ...
        # post-transformation
        # B x N x D x H x W x 3
        points = self.frustum.to(rots)[None, None, ..., None]
        # cam_to_ego
        points = torch.cat(
            (points[..., :2, :] * points[..., 2:3, :], points[..., 2:3, :]), 5)
        points = torch.inverse(img_aug).view(B, N, 1, 1, 1, 3, 3).matmul(points)
        combine = rots.matmul(torch.inverse(cam2imgs))
        points = combine.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
        points += trans.view(B, N, 1, 1, 1, 3)
        points = bda.view(B, 1, 1, 1, 1, 3,
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ...
        bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                               bev_feat_shape, interval_starts,
                               interval_lengths)
        bev_feat = bev_feat.permute(0, 1, 3, 4, 2) # B, C, Z, Y, X- > B, C, Y, X, Z
        # bev_feat = torch.cat(bev_feat.unbind(dim=2), 1)
        return bev_feat
    ...
    def voxel_pool(self, coor, depth, feat):
        B, N, D, H, W, _ = coor.shape
        C = feat.shape[2]
        x = (depth.unsqueeze(2) * feat.unsqueeze(3))
        num_points = B * N * D * H * W
        x = x.permute(0, 1, 3, 4, 5, 2).reshape(num_points, C)
        # convert coordinate into the voxel space
        coor = ((coor - self.grid_lower_bound.to(coor)) /
                self.grid_interval.to(coor))
        coor = coor.long().view(num_points, 3)
        ...
        if len(kept) == 0:
            return None, None, None, None, None
        coor, x = coor[kept], x[kept]
        x = bev_pool(x, coor, B, self.grid_size[2], self.grid_size[0], self.grid_size[1])
        x = x.permute(0, 1, 3, 4, 2) # B, C, Z, X, Y- > B, C, X, Y, Z
        return x
       ...
       #  B, N, C, H, W = input[0].shape
        ...
        if self.accelerate:
            ...
            bev_feat = self.voxel_pooling_v2(
                coor, depth,
                tran_feat)
            # bev_feat = self.voxel_pool(coor, depth, tran_feat)
        return bev_feat
