...
class CustomBasicBlock(nn.Module):
    ...
    def forward(self, x):
        if self.downsample is not None:
            identity = self.downsample(torch.cat((x[...,:1], x), dim=-1)) if self.stride==2 else self.downsample(x)
        else:
            identity = x
        x = self.conv1(torch.cat((x[...,:1], x), dim=-1)) if self.stride==2 else self.conv1(x)
        x = self.conv2(x)
        x = x + identity
        return self.relu(x)
...
class CustomResNet(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(num_layer)):
            ...
            curr_numC = num_channels[i]
            layer.extend([
                CustomBasicBlock(curr_numC, curr_numC)
                for _ in range(num_layer[i] - 1)
            ])
            layers.append(nn.Sequential(*layer))
        self.layers = nn.Sequential(*layers)
