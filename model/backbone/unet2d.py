...
class DecoderBN(nn.Module):
    ...
    def forward(self, features):
        ...
        if self.use_decoder:
            ...
            # }
            return [
                # self.resize_output_1_1(x_1_1),
                self.resize_output_1_2(x_1_2),
                self.resize_output_1_4(x_1_4),
                self.resize_output_1_8(x_1_8),
                self.resize_output_1_16(x_1_16)
            ]
        else:
            x_1_1 = features[0]
...
class UNet2D(nn.Module):
    def __init__(self, out_feature, use_decoder=True):
        super(UNet2D, self).__init__()

        basemodel_name = "tf_efficientnet_b7_ns"
        num_features = 2560

        if os.environ.get('PHIGENT', 'false') == 'true':
            repo = '../gen-efficientnet-pytorch'
            source = 'local'
        else:
            repo = "rwightman/gen-efficientnet-pytorch"
            source = 'github'
        basemodel = torch.hub.load(
            repo, basemodel_name, pretrained=True, source=source)
