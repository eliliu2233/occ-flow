...
VIZ = True
dvr = load("dvr", sources=["utils/ray_iou/lib/dvr/dvr.cpp", "utils/ray_iou/lib/dvr/dvr.cu"], verbose=True, extra_cuda_cflags=['-allow-unsupported-compiler'])
...
def main(args):
    pred_paths = sorted(glob.glob('kitti_occ/*.npz'))
    ...
    for pred_path in tqdm(pred_paths):
        frame_id = pred_path.split('/')[-1][:-4]
        if not frame_id in ego_pose_dataset.frame2scan.keys():
            continue
        scan_id = ego_pose_dataset.frame2scan[frame_id]
        batch = ego_pose_dataset[scan_id]
        ...
        gt_data = read_semantic_kitti(sample_token.split('_')[0], sample_token.split('_')[1]) #np.load(gt_filepath, allow_pickle=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default='./output/kitti_results')
