...
class ImageAug3D:
    def __init__(self, data_config, resize_lim, rot_lim, rand_flip, is_train):
    ...
    def img_transform(self, img, rotation, translation, resize, resize_dims, crop, flip, rotate):
        ...
        # img = mmcv.imresize(img, resize_dims, interpolation='bilinear')
        ...
        rotation *= resize
        translation -= np.array(crop[:2])
        if flip:
            A = np.array([[-1.0, 0.0], [0.0, 1.0]])
            b = np.array([crop[2] - crop[0], 0.0])
            rotation = A @ rotation
    ...
    def __call__(self, results):
        ...
        transforms = []
        for img in imgs:
            results['img_shape'] = self.data_config['final_dim']
            results['original_shape'] = [img.shape[0], img.shape[1]]
        ...
        results['crop'] = crop
        results['resize_dims'] = resize_dims
        return results
