...
def process_one_sample(sem_pred, lidar_rays, output_origin, flow_pred=None, return_xyz=False, sem_type='occ', sem_gt=None):
    T = output_origin.shape[1]
    pred_pcds_t = []
    if sem_type == 'sem':
        # free_id = len(occ_class_names) - 1 
        free_id = occ_class_num - 1
    ...
    for t in range(T): 
        ...
        pred_list = [pred_label, pred_dist]
        if flow_pred is not None:
            pred_flow = torch.from_numpy(flow_pred[coord_index[:, 0], coord_index[:, 1], coord_index[:, 2]])  # [N, 2]
            pred_list.append(pred_flow)

        # if return_xyz:
        #     pred_list.append(pred_pcds[0])
        
        pred_pcds = torch.cat(pred_list, dim=-1)
...
def main(args):
    ...
    for info in data_infos:
        # get reletive path
        occ_path = info['occ_path'].split('openocc_v2/')[-1]
        token2path[info['token']] = os.path.join(args.data_root, 'openocc_v2', occ_path) #'occ3d_panoptic'
    ...
    for index in tqdm(range(0,len(data_infos),1)):
        batch = ego_pose_dataset[index]
        sample_token = batch[0]
        output_origin = batch[1].to(torch.float32).unsqueeze(0)
        gt_filepath = token2path[sample_token]
        scene_name = gt_filepath.split('/')[-3]
        if not os.path.exists(os.path.join(occ_save_path, scene_name, sample_token+'.npz')):
            continue
        try:
            # pred_filepath = frames_infos[keyframe_index.index(index)]['pred_path']
            pred_filepath = os.path.join(occ_save_path, scene_name, sample_token+'.npz')
        except:
            continue
        pred_data = np.load(pred_filepath, allow_pickle=True)

        with_sem = 'sem' in return_dict
        if with_sem:
            sem_pred = pred_data['sem']
        else:
            sem_pred = pred_data['occ']
        sem_pred = np.reshape(sem_pred, [200, 200, 16])
        sem_pred = torch.from_numpy(sem_pred).permute(1,0,2)

        if 'flow' in return_dict:
            flow_pred = pred_data['flow']
            flow_pred = np.reshape(flow_pred, [200, 200, 16, 2]).transpose((1,0,2,3))*2.0
        else:
            flow_pred = None
        ...
        if 'flow' in return_dict:
            ...
            flow_gt = None
        sem_gt[..., 12:] = occ_class_num - 1
        sem_gt[:6, ...] = occ_class_num - 1
        sem_gt[-6:, ...] = occ_class_num - 1
        sem_gt[:, :6, :] = occ_class_num - 1
        sem_gt[:, -6:, :] = occ_class_num - 1
        pcd_gt = process_one_sample(sem_gt, lidar_rays, output_origin, flow_pred=flow_gt, return_xyz=VIZ, sem_type='sem')
        pcd_pred = process_one_sample(sem_pred, lidar_rays, output_origin, flow_pred=flow_pred, return_xyz=VIZ, sem_gt=sem_gt if not with_sem else None, sem_type='occ' if not with_sem else 'sem')
        ...
        data_pkl_gt[sample_token] = {
            ...
            # 'pcd_points': pcd_gt[:, 4:].astype(np.float16)
        }
        ...
        data_pkl_pred[sample_token] = {
            ...
            # 'pcd_points': pcd_pred[:, 4:].astype(np.float16)
        }
        if 'flow' in return_dict:
            data_pkl_gt[sample_token].update(pcd_flow=pcd_gt[:, 2:4].astype(np.float16))
            data_pkl_pred[sample_token].update(pcd_flow=pcd_pred[:, 2:4].astype(np.float16))
    submission_pkl_gt = {
        'method': 'GT',
        'results': data_pkl_gt
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred-occ-path", default='out/visualization/nuscenes/occ_flow/occupancy')
    # parser.add_argument("--frames-info", default='out/visualization/nuscenes/occ_flow/frames_info.pkl')
    parser.add_argument("--data-info", default='data/nuscenes_infos_val_occ.pkl')
    parser.add_argument("--output-dir", default='ray_iou_output/occ_flow')
