...
class SSCMetrics:
    ...
    def get_stats(self):
        ...
        dist.barrier()
        dist.all_reduce(self.completion_tp)
        dist.all_reduce(self.completion_fn)
        dist.all_reduce(self.completion_fp)
        dist.all_reduce(self.tps)
        dist.all_reduce(self.fps)
        dist.all_reduce(self.fns)
        dist.barrier()
        ...
        return {
            ...
            "iou": iou,
            "iou_ssc": iou_ssc,
            "iou_ssc_mean": torch.mean(iou_ssc[1:]),
        }

    def reset(self):

        self.completion_tp = torch.zeros(1, dtype=torch.float).cuda()
        self.completion_fp = torch.zeros(1, dtype=torch.float).cuda()
        self.completion_fn = torch.zeros(1, dtype=torch.float).cuda()
        self.tps = torch.zeros(self.n_classes, dtype=torch.float).cuda()
        self.fps = torch.zeros(self.n_classes, dtype=torch.float).cuda()
        self.fns = torch.zeros(self.n_classes, dtype=torch.float).cuda()

        # self.hist_ssc = torch.zeros((self.n_classes, self.n_classes)).cuda()
        # self.labeled_ssc = 0
        # self.correct_ssc = 0
    ...
    def get_score_completion(self, predict, target, nonempty=None):
        ...
        b_pred = torch.zeros(predict.shape).cuda()
        b_true = torch.zeros(target.shape).cuda()
        b_pred[predict > 0] = 1
        b_true[target > 0] = 1  
        ...
        for idx in range(_bs):
            ...
            fp = torch.logical_and(y_true != 1, y_pred == 1).sum()
            fn = torch.logical_and(y_true == 1, y_pred != 1).sum()
            tp_sum += tp
            fp_sum += fp
            fn_sum += fn
    ...
    def get_score_semantic_and_completion(self, predict, target, nonempty=None):
        ...
        tp_sum = torch.zeros(_C, dtype=torch.int32).cuda()  # tp
        fp_sum = torch.zeros(_C, dtype=torch.int32).cuda()  # fp
        fn_sum = torch.zeros(_C, dtype=torch.int32).cuda()  # fn
        ...
        for idx in range(_bs):
            ...
            if nonempty is not None:
                nonempty_idx = nonempty[idx, :].reshape(-1)
                y_pred = y_pred[
                    torch.logical_and(nonempty_idx == 1, y_true != 255)
                ]
                y_true = y_true[
                    torch.logical_and(nonempty_idx == 1, y_true != 255)
                ]
            for j in range(_C):  # for each class
                tp = torch.logical_and(y_true == j, y_pred == j).sum()
                fp = torch.logical_and(y_true != j, y_pred == j).sum()
                fn = torch.logical_and(y_true == j, y_pred != j).sum()
