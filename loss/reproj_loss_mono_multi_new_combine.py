...
class ReprojLossMonoMultiNewCombine(BaseLoss):
    ...
    def reproj_loss(
        ...
        for cam, (ray_idx, weight, t) in enumerate(zip(ray_indices, weights, ts)):
            ...
            if torch.isnan(proj_loss_avg).sum()>0:
                debug = True
...
class ReprojLossMono(BaseLoss):
    ...
    def reproj_loss(
        ...
        bs, num_cams = curr_imgs.shape[:2]
        assert bs == 1
        depth = F.interpolate(ms_depths[0].reshape(num_cams, 1, *self.ray_resize), size=self.img_size, mode='bilinear', align_corners=False)
        # prepare transformation matrices
        img2prevImg, img2nextImg = [], []
        for meta in metas:
            img2prevImg.append(meta['img2prevImg'])
        ...
        tot_loss = 0.
        mesh_grid = np.meshgrid(range(self.img_size[1]), range(self.img_size[0]), indexing='xy')
        ray_coords = torch.from_numpy(np.stack(mesh_grid, axis=-1)).float().to(device).reshape(-1, 2)
        pixel_coords = torch.ones((num_cams, len(ray_coords), 4), device=device) # B, N, R, 4
        pixel_coords[..., :2] = ray_coords.unsqueeze(0).repeat(num_cams, 1, 1)
        pixel_coords[..., :3] *= depth.reshape(num_cams, -1, 1)
        pixel_coords = pixel_coords.unsqueeze(-1)
        ...
        def cal_pixel(trans, coords):
            ...
            pixel = pixel[..., :2] / torch.maximum(torch.ones_like(pixel[..., :1]) * eps, pixel[..., 2:3])
            return pixel
        pixel_prev = cal_pixel(img2prevImg.flatten(0, 1), pixel_coords).reshape(num_cams, *self.img_size, 2)
        pixel_next = cal_pixel(img2nextImg.flatten(0, 1), pixel_coords).reshape(num_cams, *self.img_size, 2)

        def sample_pixel(pixel, imgs):
            # imgs: B, N, 3, H, W
            # pixel: B, N, 1, R, 2
            pixel = pixel.clone()
            pixel[..., 0] /= (self.img_size[1] - 1)
            pixel[..., 1] /= (self.img_size[0] - 1)
            pixel = 2 * pixel - 1
            pixel_rgb = F.grid_sample(
                imgs, 
                pixel, 
        ...
        rgb_prev = sample_pixel(pixel_prev, prev_imgs.flatten(0, 1))
        rgb_next = sample_pixel(pixel_next, next_imgs.flatten(0, 1))
        ...
        reprojection_loss = torch.cat((compute_reprojection_loss(rgb_prev, curr_imgs.flatten(0, 1)), 
                                 compute_reprojection_loss(rgb_next, curr_imgs.flatten(0, 1))), dim=1)
        if not self.no_automask:
            identity_reprojection_loss = torch.cat((compute_reprojection_loss(prev_imgs.flatten(0, 1), curr_imgs.flatten(0, 1)), 
                                                    compute_reprojection_loss(next_imgs.flatten(0, 1), curr_imgs.flatten(0, 1))), dim=1)
            identity_reprojection_loss += torch.randn(identity_reprojection_loss.shape).to(device) * 1e-5
            proj_loss = torch.cat((reprojection_loss, identity_reprojection_loss), dim=1)
        else:
            proj_loss = reprojection_loss
        proj_loss, _ = torch.min(proj_loss, dim=1)
