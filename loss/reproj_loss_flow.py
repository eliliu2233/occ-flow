...
class ReprojLossFlow(BaseLoss):
    ...
    def reproj_loss(
        ...
        dynamic_dis = dynamic_mask is not None
        assert bs == 1
        ...
        tot_loss = 0.
        for cam, (ray_idx, weight, t, next_warp) in enumerate(zip(ray_indices, weights, ts, points_warps)): #prev_warps,
            ...
            dyn_mask_ = torch.gather(dynamic_mask[:, cam:(cam+1), ...].flatten(), -1, ray_idx) if dynamic_mask is not None else torch.zeros_like(weight, dtype=bool, device=device)
            if sampled_flows is not None:
                sampled_flow = sampled_flows[cam].reshape(1,1,-1,2).clone()
                if dynamic_dis:
                    sampled_flow[~dyn_mask_.reshape(1,1,-1)] = 0.0
                next_warp = next_warp + torch.cat([sampled_flow, torch.zeros_like(sampled_flow[...,0:1])],dim=-1)
            ...
            diff = diff_next
            cnt = next_mask.to(torch.float)
            if dynamic_mask is not None:
                static_mask = (~dynamic_mask[:,cam:cam+1,...]).flatten()
                pixel_prev, prev_mask = cal_pixel(img2prevImg[:, cam:(cam+1), ...], pixel_coords)
                ...
                prev_mask = prev_mask & (torch.gather(static_mask, dim=0, index=ray_idx))
                diff_prev[~prev_mask] = 0.
                cnt += prev_mask.to(torch.float)
                diff += diff_prev
            ...
            if not self.no_ssim:
                ...
                rgb_combine_ = rgb_next
                if dynamic_mask is not None:
                    rgb_prev[~prev_mask] = 0.
                    rgb_combine_ += rgb_prev
