...
def get_smooth_loss(disp, img):
    ...
    """
    grad_disp_x = torch.mean(torch.abs(disp[:, :, :, :-1] - disp[:, :, :, 1:]), 1, keepdim=True)
    grad_disp_y = torch.mean(torch.abs(disp[:, :, :-1, :] - disp[:, :, 1:, :]), 1, keepdim=True)
...
class EdgeLoss3DMS(BaseLoss):
    ...
    def edge_loss(self, curr_imgs, ms_depths, ms_rays=None, ms_accs=None, max_depths=None):
        # curr_imgs: B, N, C, H, W
        # depth: B, N, R
        # rays: R, 2
        if self.use_inf_mask:
            assert ms_accs is not None and max_depths is not None
        if ms_rays is not None and not isinstance(ms_rays, list):
            ms_rays = [ms_rays] * len(ms_depths)
        bs, num_cams, num_rays = ms_depths[0].shape

        tot_loss = 0.
        for scale, depth in enumerate(ms_depths):
            if ms_rays is not None:
                pixel_curr = ms_rays[scale].clone().reshape(1, 1, num_rays, 2).repeat(
                ...
                rgb_curr = F.interpolate(curr_imgs.flatten(0, 1), size=self.ray_resize, mode='bilinear', align_corners=False)

            if self.use_inf_mask:
                depth = depth * ms_accs[scale] + max_depths[scale] * (1 - ms_accs[scale])
            depth = depth.reshape(bs * num_cams, 1, *self.ray_resize)
            depth = 1.0 / (depth + 1e-7)    
...
class EdgeLossFlow(BaseLoss):
    ...
    def edge_loss(self, curr_imgs, scene_flows, ms_rays):
        # curr_imgs: B, N, C, H, W
        # depth: B, N, R
        # rays: R, 2
        bs, num_cams, num_rays, _ = scene_flows.shape
        pixel_curr = ms_rays.clone().reshape(1, 1, num_rays, 2).repeat(bs * num_cams, 1, 1, 1) # bs*N, 1, R, 2
        pixel_curr[..., 0] /= self.img_size[1]
        pixel_curr[..., 1] /= self.img_size[0]
        pixel_curr = pixel_curr * 2 - 1
        rgb_curr = F.grid_sample(
        ...
        rgb_curr = rgb_curr.reshape(bs * num_cams, -1, *self.ray_resize)
        # (B*N, 2, H, W)
        scene_flows = scene_flows.reshape(bs * num_cams, *self.ray_resize, 2).permute(0,3,1,2)
        smooth_loss = get_smooth_loss(scene_flows, rgb_curr)
