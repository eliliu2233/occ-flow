...
class AdaptiveSparsityLoss(BaseLoss):
    ...
    def __init__(self, weight=1, input_dict=None, slack=3.0, **kwargs):
        super().__init__(weight)
...
class OccEntropyLoss(BaseLoss):
    ...
    def entropy_loss(self, occupancy):
        return (-occupancy * torch.log(occupancy + 1e-8)).mean()
